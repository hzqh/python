...
class Clienttest3():
    ...
    def start(self):
        ...
        host = eval(cf.get('akm', 'host')) 
        BUFSIZE = eval(cf.get('akm', 'BUFSIZE')) 
        port = eval(cf.get('akm', 'port'))
        KeyID =  eval(cf.get('client1', 'KeyID'))
        
        conn = MySQLdb.connect(host=eval(cf.get('db3', 'host')),user=eval(cf.get('db3', 'user')),passwd=eval(cf.get('db3', 'passwd')),db=eval(cf.get('db3', 'db')),port=eval(cf.get('db3', 'port')),charset='utf8')
        cur = conn.cursor()
        hero = DBConnection.DBConn(eval(cf.get('db3', 'db')),conn,cur)
        ...
        user = eval(cf.get('client1', 'UserName'))                         # 用户名
