...
class   RecvPacket():
    ...
    def unpack(self,recvData):    
        ...
        #print 'rc_packet sess_id:申请创建应用密钥反馈报文的SessionID',SessionID
        if(self.key_typ==0 and self.automatic ==1):
            print 'key_typ==0 and automatic ==1 '
            if(self.status==0):
                left,self.sess_id=struct.unpack('!4s16s',self.data)
            ...
            if(self.status == 0):
                ...
                self.key_id_2=array.array('B',self.key_id_2)
...
class   ObtRecvPacket():
    ...
    def get_key_list(self):
        if(self.encry_alg_2==1):
            key_id_2_pack=struct.pack('%dB'%len(self.key_id_2),self.key_id_2)
            key_list_MD5_1 = tools.md5Encode(key_id_2_pack)
            key_id_rev=self.key_id_2[::-1]
            key_id_2_pack_2=struct.pack('%dB'%len(key_id_rev),key_id_rev)
            key_list_MD5_2 = tools.md5Encode(key_id_2_pack_2)
            sess_key_pack_hex=key_list_MD5_1+key_list_MD5_2
            self.sess_key=binascii.unhexlify(sess_key_pack_hex)
            print 'sess key',self.sess_key
            key_list_pack= struct.pack('%dB'%len(self.key_list),self.key_list)       
            self.key_list=self.data_decry_pack(key_list_pack)
        return self.key_list
...
class   ModRecvPacket():
    ...
    def unpack(self,recvData):    
        ...
        self.msg_typ,self.req_id,self.status,self.sess_id=struct.unpack('!BHB16s',self.data)
        self.sess_id=array.array('B',self.sess_id)
        print 'self.msg_typ~~~修改应用密钥反馈报文',self.msg_typ
        print 'self.status~~~修改应用密钥反馈报文',self.status
    def getResult(self):
        return self.status
